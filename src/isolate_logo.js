...
// M&D AI Academy logo location
const MDAI_LOGO = '/Users/vquinones/Q Dropbox/MDAI.Academy/Dev/Projects/MDAI-Web/MD-website/context-docs/images/Logo.png';

async function process() {
    // Try MDAI logo first, then fall back to assets folder
    let sourceFile = INPUT_FILE;

    if (fs.existsSync(MDAI_LOGO)) {
        sourceFile = MDAI_LOGO;
        console.log(`Using existing MDAI logo: ${MDAI_LOGO}`);
    } else if (!fs.existsSync(INPUT_FILE)) {
        console.error(`Error: No logo found. Please place your logo at:`);
        console.error(`  - ${INPUT_FILE}`);
        console.error(`  - Or: ${MDAI_LOGO}`);
        process.exit(1);
    }
