...
const OUTPUT_DIR = path.join(__dirname, '../mdai-logo-kit');
const LOGO_FOLDER = '/Users/vquinones/Q Dropbox/MDAI.Academy/Dev/Projects/mdai-logo-generator/Logos';

// M&D AI Academy Brand Colors
const BRAND_PRIMARY = '#2563EB';   // Royal Blue (education, trust)
const BRAND_SECONDARY = '#10B981'; // Emerald Green (growth, AI)
const BRAND_ACCENT = '#F59E0B';    // Amber (energy, learning)
...
const getFileName = (config, variant, platform, sizeName, width, height, ext) => {
    ...
    return `MDAI_${variantStr}_${finalPlatform}_${sizeStr}.${ext}`;
};

// --- MAIN ---

async function processImages() {
    console.log(`Starting MDAI asset generation from ${SOURCE_FILE}...`);
    ...
    for (const size of CONFIG.favicon) {
        const fileName = `MDAI_favicon_${size.w}.png`;
    ...
    const keyAssets = [
        path.join(OUTPUT_DIR, 'app', 'MDAI_full_store_1024.png'),
        path.join(OUTPUT_DIR, 'web', 'MDAI_full_og_1200x630.png'),
        path.join(OUTPUT_DIR, 'social', 'MDAI_full_instagram_1080.png'),
    ...
    console.log(`\n✅ MDAI asset generation complete!`);
